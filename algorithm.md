...
- **The contradiction check** happens before anything is written. It
  ...
  records, and the cells it wants to remove against the "blocked by"
  records. If nothing contradicts, the anchor writes. If something does, the
...
| the records | `record_seen`, `Seen`, `SeenRecord` |
...
document, restart counts included; `oracle.rs` is the consistency check of
